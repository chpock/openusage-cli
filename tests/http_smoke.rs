...
async fn http_api_smoke_for_plugins_and_usage_refresh() {
    ...
    assert_eq!(
        ...
        serde_json::json!({"id": "default", "displayName": "default"}),
...
async fn single_provider_returns_sole_custom_account() {
    ...
    assert_eq!(
        single_json["account"],
        serde_json::json!({"id": "my-account", "displayName": "My Account"}),
        "single provider must return the custom account"
    );
        ...
        entry_script: r#"
            globalThis.__openusage_plugin = {
                ...
                discoverAccounts(ctx) {
                    return [
                        { id: "alpha", displayName: "Alpha Account" },
                        { id: "beta", displayName: "Beta Account" }
                    ];
                }
            };
        "#
