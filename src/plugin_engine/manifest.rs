...
pub fn load_plugins_from_dir(plugins_dir: &std::path::Path) -> Vec<LoadedPlugin> {
    ...
    log::debug!("scanning plugins dir: {}", plugins_dir.display());
    ...
    for entry in entries.flatten() {
        ...
        log::debug!("found plugin candidate dir: {}", path.display());

        let manifest_path = path.join("plugin.json");
        if !manifest_path.exists() {
            log::debug!("skipping candidate {}: missing plugin.json", path.display());
            continue;
        }
        ...
        match load_single_plugin(&path) {
            ...
            Err(err) => {
                log::warn!("failed to load plugin at {}: {}", path.display(), err);
            }
        }
    }
    ...
    log::debug!(
        "plugin scan complete: {} plugin(s) loaded from {}",
        plugins.len(),
        plugins_dir.display()
    );
    plugins
...
fn load_single_plugin(
    ...
    log::debug!("loading plugin from {}", plugin_dir.display());
